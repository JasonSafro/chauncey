...
function gym_remove_images($posttext, $echo = true)
{
    $posttext1 = $posttext;
    ...
    $only_post_text = gym_remove_empty_tags($only_post_text);
    
    if ($echo) echo $only_post_text; else return $only_post_text;
}

function gym_remove_empty_tags($html)
{
$pattern = "/<[^\/>]*>([\s]?)*<\/[^>]*>/";
  ...
  function get_gallery_page_link($post_id)
{
    ...
    WHERE post_content LIKE '%[gym_gallery%' AND post_content LIKE '%cat_id%' AND post_content LIKE '%$parent%' AND post_type='page'");
...
function gym_excerpt($excerpt_length)
{
...
function newgravatar ($avatar_defaults) {
     ...
     $avatar_defaults[$myavatar] = "Gym Avatar";
...
function my_scripts_method() {
    ...
    if ($layout == "stretched") wp_enqueue_style("style-wide", SYSTEM_ROOT."/css/style-wide.css");
    ...
    if ($options['scroll_control'] != '0') 
    {
        wp_enqueue_script("gym_header", SYSTEM_ROOT."/javascript/header.js");
    }
...
function gym_contact_page_create_field($gym_contact_page_field, $gym_contact_page_field_title, $gym_contact_page_field_required, $gym_contact_page_field_select)
{
    global $_POST;
    $required = "";
    $required_class = "";
    if ($gym_contact_page_field != "Nothing")
    {
        if ($gym_contact_page_field_required == "yes") 
        {
            $required = "<em>(".__('*', SYSTEM_THEME_SHORT).")</em>";
            $required_class = "requiredField";
        }
        if ($gym_contact_page_field_required == "yes_email") 
        {
            $required = "<em>(".__('*', SYSTEM_THEME_SHORT).")</em>";
            $required_class = "requiredField email";
        }
        
        $field_name = "gym_".only_characters($gym_contact_page_field_title);
    ?>
                <li <?php if ($gym_contact_page_field == "textarea") echo "class='textarea'" ?>>
                    <p><strong><?php echo $gym_contact_page_field_title; ?></strong> <?php echo $required; ?></p>
                    <?php 
                    if ($gym_contact_page_field == "text")
                    {
                    ?>
                    <input name="<?php echo $field_name; ?>" type="text" class="<?php echo $required_class; ?> gym_c_form" />
                    <?php
                    }
                    ?>
                    
                    <?php 
                    if ($gym_contact_page_field == "textarea")
                    {
                    ?>
                    <textarea name="<?php echo $field_name; ?>" rows="20" cols="30" class="<?php echo $required_class; ?> gym_c_form"></textarea>    
                    <?php
                    }
                    ?>
                    
                    <?php 
                    if ($gym_contact_page_field == "select")
                    {
                        $gym_contact_page_field_select_array = explode(",", $gym_contact_page_field_select);
                    ?>              
                    <select name="<?php echo $field_name; ?>" class="<?php echo $required_class; ?> gym_c_form">
                    <option></option>
                      <?php
                      foreach ($gym_contact_page_field_select_array as $option)
                      {
                      ?>
...
function gym_get_images($posttext)
{
    // We will search for the src="" in the post content
    $regular_expression = '~src="[^"]*"~';
    $regular_expression1 = '~<img [^\>]*\ />~';
